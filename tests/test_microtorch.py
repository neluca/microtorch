...
def test_concat_with_square():
    ...
    # Perform stack operation in pytorch
    z_t = torch.concat([x_t, y_t], dim=0)
    z_t = z_t ** 2  # Square the tensor
    z_t.sum().backward()
    ...
    # Perform stack operation in microtorch
    z_mt = microtorch.concat([x_mt, y_mt], dim=0)
    z_mt = z_mt ** 2  # Square the tensor
    z_mt.sum().backward()
    ...
    assert np.allclose(
        z_t.detach().numpy(), z_mt.data
    ), "Concat operation results do not match between pytorch and microtorch."
